...
import LastStudents from "../LastStudents";
...
describe("Last Student mock selector", () => {
  let mock;
  ...
  it("should render a loading message", async () => {
    ...
    await waitFor(() => renderWithProvider(<LastStudents />));
    screen.getByText(/Loading.../i);
  });

  it("should render a empty student list message", async () => {
    mock.onGet(`${URL_API}students/scores/current`).reply(200, []);
    await waitFor(() => renderWithProvider(<LastStudents />));
    screen.getByText(/No students found!/i);
  });

  it("should render a student list", async () => {
    mock.onGet(`${URL_API}students/scores/current`).reply(200, students);
